...
saveRDS(standardization_info,
    file = paste0(
        ...
        "ncgp_total_minus_gaze_standardization_info.rds"
    )
)
...
model_name <- "ncgp_total_minus_gaze"
    ...
    m <- brm(
        bf(
            rt_total_minus_gaze ~ log_freq_z + word_nchar_z + surprisal_nc_z + word_num_in_sent + pos_tag + Region * cond_id +
                (log_freq_z + word_nchar_z + surprisal_nc_z + word_num_in_sent || submission_id) +
                (1 || Item),
            hu ~ log_freq_z + word_nchar_z + surprisal_nc_z + word_num_in_sent + pos_tag
        ),
        data = dat,
...
#    std_info <- readRDS("outputs/fitted_models/ncgp_total_minus_gaze_standardization_info.rds")
